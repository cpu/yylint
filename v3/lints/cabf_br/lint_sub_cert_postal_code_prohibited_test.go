...
func TestSubCertPostalCodeProhibited(t *testing.T) {
	inputPath := "subCertProvinceMustNotAppear.pem"
	expected := lint.Error
	out := test.TestLint("e_sub_cert_postal_code_must_not_appear", inputPath)
...
func TestSubCertPostalCodeNotProhibited(t *testing.T) {
	inputPath := "subCertPostalCodeNotProhibited.pem"
	expected := lint.Pass
	out := test.TestLint("e_sub_cert_postal_code_must_not_appear", inputPath)
	if out.Status != expected {
		t.Errorf("%s: expected %s, got %s", inputPath, expected, out.Status)
